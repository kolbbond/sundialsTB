% 230305 - modified IDAMonitor function
%           moving figures, need to figure out error ???
function [new_data] = IDAMonitorCustom(call, T, Y, YQ, YS, data)
...
global debugout;
if(debugout) 
    %disp('in IDAMonitorCustom');
end

if(debug_clf)
    %disp('in IDAMonitorCustom'); disp(['call: ', num2str(call)]); 
end
...
% Initialize unspecified fields to default values.
  ...
  if(debug_clf)
    disp('in IDAMonitorCustom'); disp(data.hft); 
  end
...
if(debugout)
    disp(['in text init figure: ']); %disp(hft);
end
figure(hft); movegui([950 200]);
